...
[n_Amp,n_Ph] = toroidal_n_mode(t,x,test_signal); % get amplitude and phase
...
function [n_Amp,n_Ph] = toroidal_n_mode(t,x,signal)
    t_start = t(1);
    t_end = t(end);
    len_freq = length(x);
    ...
    for i = 1:length(t)
        sig = interp1(x,signal(i,:),x_interp,'makima','extrap' );
        y_fft = fft(sig);
