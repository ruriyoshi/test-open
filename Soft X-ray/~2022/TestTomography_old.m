function [error_high_value,error_low_value] = TestTomography_old(NL)
...
VectorImage = AssumedProfile(gm2d,N_grid,true);
VectorImage_high = VectorImage.*1.03;
VectorImage_low = VectorImage.*0.97;

%         再構成計算
EE = clc_distribution(M,K,gm2d1,U1,s1,v1,VectorImage,plot_flag,NL);
EE_high = clc_distribution(M,K,gm2d1,U1,s1,v1,VectorImage_high,plot_flag,NL);
EE_low = clc_distribution(M,K,gm2d1,U1,s1,v1,VectorImage_low,plot_flag,NL);
...
error_high_image = EE_high - EE;
error_low_image = EE_low - EE;
...
function Iwgn = AssumedProfile(gm2d,range,plot_flag)

[N_projection, N_g] = size(gm2d);
