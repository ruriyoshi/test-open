...
for i=1:128
    bz1(:,i) = filter(bb,aa,b1(:,i));
    bz1(:,i) = bz1(:,i) - mean(b1(1:40,i));
    if(ch1(i) > 0)
    ok_bz1(ch1(i))=ok1(i);
...
r_shift = 0.00;
zprobepcb    = [-0.0525,-0.021,0,0.0525,0.021];
rprobepcb    = [0.06,0.08,0.1,0.12,0.14,0.15,0.155,0.16,0.165,0.17,0.175,0.18,0.185,0.19,0.195,0.2,0.205,0.21,0.215,0.22,0.225,0.23,0.235,0.24,0.245];
...
for i = 1:length(ok_bz1)
   
    index_r = (abs(rpos_bz1(i)-rprobepcb)<0.001);index_z = (zpos_bz1(i)==zprobepcb);
    ok_bz_matrix = ok_bz_matrix + rot90(index_r,-1)*index_z*ok_bz1(i);
end
...
filename = strcat('C:\Users\uswk0\OneDrive - g.ecc.u-tokyo.ac.jp\data\before_picture\a038_',num2str(shot),'.mat');
save(filename)
