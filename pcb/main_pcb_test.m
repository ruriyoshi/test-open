...
%%%midplaneとかO点、X点を探す
% [psimid,mid]=min(data2D.psi(:,10:40,:),[],2);
% mid=mid+9;
% mid(mid==10)=1;
% mid(mid==40)=n;

midlog=false(n,n,numel(trange));
midlog=islocalmin(data2D.psi,2,'MaxNumExtrema',1);
midlog(:,1,:)=false;
midlog(:,n,:)=false;
...
%両端を除いても極小が2つある場合は考慮できていない
psimid=zeros(n,1,numel(trange));
mid=psimid;
index=repmat(1:n,n,1);
for i=1:numel(trange)
psi_store=data2D.psi(:,:,i);
psimid(:,:,i)=psi_store(midlog(:,:,i));
mid(:,:,i)=index(midlog(:,:,i));
end
%index=repmat(1:n,n,1,numel(trange));
%mid=index(midlog);
...
% %figureの保存
% saveas(gcf,strcat(pathname.save,'\pcb_IDX',num2str(IDX),'_',num2str(t_start),'us2.png'))
