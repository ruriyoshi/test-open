%% 
%未完成→抵抗率のばらつきが大きいので修正必要、X点の位置はこれでいいのか
% CAOプローブのデータからX点でのpsi_commonや電流密度、電場、抵抗を時系列で求めるコード
% 
...
caoEt=zeros(frame,numel(IDXlist));
caoJt=caoEt;
caoRes=caoEt;
...
tstart=472;
tend=482;
[caoEt(:,i),caoJt(:,i),caoRes(:,i),merge_ind(:,i)]=xdata(T, IDX, pathname,start,frame,tstart,tend);
end


function [xEt,xJt,xRes,merge_ind]=xdata(T, IDX, pathname,start,frame,tstart,tend)
date=T.date(IDX);
shot=T.shot(IDX);
...
% 
% subplot(4,1,3)
% plot(time,xRes.*1e+3,'-o','Color','k','LineWidth',1)
% %yline(0,'k-')
% xline(472,'b-',"LineWidth",1)
% ylabel('Resistivity at Xpoint [mΩ・m]','FontSize',13,'Color','b')
% xlabel('Time [us]','FontSize',13)
% xlim([time(merge_ind) time(merge_ind)+20])
% ylim([0 2])
% %set(gca,'FontSize',13);
