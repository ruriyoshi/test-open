...
for m=1:Nplot %図示する時間
    ...
    elseif Nplot < 4
        ...
        close(fig)
        warning('Input error in Nplot.')%Nplotの入力エラー
        return;
    end
    if cmap
        switch cmap
            case 'Bz'
                contourf(grid2D.zq(1,:),grid2D.rq(:,1),data2D.Bz(:,:,i),30,'LineStyle','none')%Bz
                clim([-0.01,0.01])%Bz
                if cbar
            ...
            case 'psi'
                contourf(grid2D.zq(1,:),grid2D.rq(:,1),data2D.psi(:,:,i),40,'LineStyle','none')%psi
                clim([-5e-3,5e-3])%psi
                if cbar
                    colorbar('Location','eastoutside')
                    c = colorbar;
