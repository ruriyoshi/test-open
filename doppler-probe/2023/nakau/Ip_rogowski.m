function [t,I] = Ip_rogowski(date,aquisition_rate,offset,shot,plot_flag)
...
path_txt = strcat(path,'.txt');
path_rgw = strcat(path,'.rgw');

if ~isfile(path_rgw)
    t = NaN;
    I = NaN;
    return
elseif ~isfile(path_txt)
...
a = 1;
smoothed_and_filtered = filter(b,a,smoothed);

if plot_flag
    figure;
    plot(x/aquisition_rate+offset,smoothed_and_filtered,'LineWidth',4);
%     hold on
...
% t = x/aquisition_rate+offset;
% I = data(x,i+2)*calibration(i);
t = x/aquisition_rate+offset;
I = smoothed_and_filtered*460;
...
% 
% figure;
% plot(x/aquisition_rate+offset,data(x,2+2)*calibration(2),'LineWidth',2);
% hold on
...
function [date_str,shot_str,path] = directory_generation_Rogowski(date,shot)
    date_str = num2str(date);
    if shot < 10
%         path = [folder_directory_rogo,date_str,'/',date_str,'00',num2str(shot),'.txt'];
        path = [folder_directory_rogo,date_str,'/',date_str,'00',num2str(shot)];
        shot_str = ['00',num2str(shot)];
    elseif shot < 100
%         path = [folder_directory_rogo,date_str,'/',date_str,'0',num2str(shot),'.txt'];
        path = [folder_directory_rogo,date_str,'/',date_str,'0',num2str(shot)];
        shot_str = ['0',num2str(shot)];
    elseif shot < 1000
%         path = [folder_directory_rogo,date_str,'/',date_str,num2str(shot),'.txt'];
        path = [folder_directory_rogo,date_str,'/',date_str,num2str(shot)];
