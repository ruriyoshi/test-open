...
%     contourf(grid2D.zq(1,:),grid2D.rq(:,1),data2D.Bz(:,:,i),30,'LineStyle','none')
    ...
    axis image
    axis tight manual
    caxis([-1.4*1e+6,1.4*1e+6]) %jt%カラーバーの軸の範囲
%     caxis([-3*1e+6,1.5*1e+6]) %カラーバーの軸の範囲
%     caxis([-0.04,0.04])%Bt
...
%  saveas(gcf,strcat('C:\Users\kuru1\OneDrive - g.ecc.u-tokyo.ac.jp\labo\experiment\230127\jt\a038_',num2str(shot),'_jt_t461us.png'))
%  close
