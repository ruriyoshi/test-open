function [] = plot_psi_multi(B_z,r_probe,z_probe,times,fitting,fill,fixed_Clayer,show_probe,shot,show_xpoint,colum_num)
...
for t = times
    ...
    if show_xpoint
       ...
       %x_point_index = logical(x_point_index);
        xpoint_z = zeros(1,1024);
        xpoint_r = zeros(1,1024);

        for i=1:1024
            xpoint_z(1,i) = probe_mesh_z(x_point_index(:,:,i));
            xpoint_r(1,i) = probe_mesh_r(x_point_index(:,:,i));
        end

        plot(xpoint_z(1,t),xpoint_r(1,t),'x','color','r','markersize',8);
    end
